...
(async () => {
  // const user = await getUser('+12532937820');
  const res = await parseCommand('status left', "+12532937820");
  const res1 = await parseCommand('close left', "+12532937820");
  const res2 = await parseCommand('status left', "+12532937820");
  const res3 = await parseCommand('open left', "+12532937820");
...
// I have a question, now with the State Machine we have implemented, if the door is being opened "opening", having that in the database no matter what the user sends he gets a respond back along the lines of "The door is currently ${opening/closing}. Please wait until the process is done and try again." Do I have to make it a way so the user can send like "open_compelete/close_complete" or we will implement that later with the electronics and sensors? 